...
function outputClients(){
  let html=clientDatabase.map(function(data, i){
    return '<tr data-index='+i+'><td>'+data.name+'</td><td>'+data.email+'</td><td>'+data.phone+'</td><td>'+data.mobile+'</td><td>'+data.address+'</td><td>'+data.billing+'</td><td>'+data.business+'</td><td>'+data.notes+'</td><td><button class="deleteEntry"></button></td></tr>'
  })
